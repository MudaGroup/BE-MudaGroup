...
export const getBuilding = async(req, res) => {
    try {
        const response = await buildingModel.findAll();
        res.json(response);
    } catch (error) {
...
export const getBuildingById = async(req, res) => {
    try {
        const response = await buildingModel.findOne({
            where: {
                id: req.params.id,
            },
        });
...
export const saveBuilding = (req, res) => {
    ...
    const url = `${req.protocol}://${req.get("host")}/image/building/${fileName}`;
    ...
    file.mv(`./public/image/building/${fileName}`, async(err) => {
...
export const updateBuilding = async(req, res) => {
    const product = await buildingModel.findOne({
        where: {
            id: req.params.id,
        },
    });
    ...
    if (req.files || !req.files.file) {
        ...
        const filepath = `./public/image/building/${product.image}`;
        ...
        file.mv(`./public/image/building/${fileName}`, (err) => {
            if (err) return res.status(500).json({ msg: err.message });
        });
    }
